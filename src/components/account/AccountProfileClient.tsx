...
export function AccountProfileClient({ user }: Props) {
  ...
  const [profile, setProfile] = React.useState<UserProfile>(() => publicUserToProfileForm(user));
  ...
  const handleResetProfile = () => {
    setProfile(publicUserToProfileForm(user));
    notify.info("คืนค่าจากข้อมูลล่าสุดบนเซิร์ฟเวอร์");
  };
  ...
  const planLabel = user.plan === "monthly" ? "Pro Monthly" : user.plan === "yearly" ? "Pro Yearly" : "Free";
  const planDurationDays = dayDiffCeil(user.planStartedAt, user.planExpiresAt);
  const planRemainingDays = dayDiffCeil(new Date().toISOString(), user.planExpiresAt);

  return (
    <div className="mx-auto grid max-w-3xl gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-zinc-900">โปรไฟล์</h1>
          <p className="mt-1 text-sm text-zinc-500">
            ข้อมูลส่วนตัวซิงก์กับบัญชีบนเซิร์ฟเวอร์ — เปลี่ยนรหัสผ่านได้จากปุ่มข้างล่าง
          </p>
        </div>
        <Link
          href="/settings"
        ...
        <div className="px-5 py-6 sm:px-8">
          <div className="mb-5 rounded-2xl border border-emerald-200/70 bg-emerald-50/50 p-4">
            <div className="flex flex-wrap items-center gap-2">
              <div className="text-sm font-semibold text-zinc-900">แพ็กเกจสมาชิก</div>
              <span className="rounded-full border border-emerald-200/80 bg-white px-2.5 py-0.5 text-xs font-medium text-emerald-700">
                {planLabel}
              </span>
            </div>
            {user.plan === "free" ? (
              <div className="mt-2 text-sm text-zinc-600">แผนฟรี (ไม่มีวันหมดอายุ)</div>
            ) : (
              <div className="mt-3 grid gap-1 text-sm text-zinc-700 sm:grid-cols-2">
                <div>
                  เริ่มแพ็กเกจ: <span className="font-medium">{formatThaiDate(user.planStartedAt)}</span>
                </div>
                <div>
                  หมดอายุ: <span className="font-medium">{formatThaiDate(user.planExpiresAt)}</span>
                </div>
                <div>
                  ระยะแพ็กเกจ:{" "}
                  <span className="font-medium">
                    {typeof planDurationDays === "number" ? `${planDurationDays} วัน` : "-"}
                  </span>
                </div>
                <div>
                  วันคงเหลือ:{" "}
                  <span className="font-medium">
                    {typeof planRemainingDays === "number" ? `${planRemainingDays} วัน` : "-"}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className="grid gap-1.5 sm:col-span-2">
              <label className="text-xs font-medium text-zinc-500" htmlFor="displayName">
                ชื่อที่แสดง
              </label>
              <Input
                id="displayName"
            ...
            <div className="grid gap-1.5">
              <label className="text-xs font-medium text-zinc-500" htmlFor="lineId">
                LINE / ช่องทางติดต่อ
              </label>
              <Input
                id="lineId"
                value={profile.lineId}
                onChange={(e) => setProfile((p) => ({ ...p, lineId: e.target.value }))}
                placeholder="@line_id หรือลิงก์"
              />
            </div>
            <div className="grid gap-1.5">
              <label className="text-xs font-medium text-zinc-500" htmlFor="country">
                ประเทศ / ภูมิภาค
              </label>
              <Select
                id="country"
            ...
            <div className="grid gap-1.5">
              <label className="text-xs font-medium text-zinc-500" htmlFor="occupation">
                อาชีพ / บทบาท
              </label>
              <Input
                id="occupation"
            ...
            <div className="grid gap-1.5 sm:col-span-2">
              <label className="text-xs font-medium text-zinc-500" htmlFor="notes">
                โน้ตส่วนตัว
              </label>
              <Textarea
                id="notes"
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2">
            ...
            <Button type="button" variant="ghost" className="text-zinc-600" onClick={handleResetProfile}>
              คืนค่า
            </Button>
          </div>
        </div>
      ...
      <Modal open={pwModalOpen} onClose={closePwModal} title="เปลี่ยนรหัสผ่าน" className="max-w-md">
        <p className="mb-4 text-xs leading-relaxed text-zinc-500">
          ส่งไปที่ <code className="rounded bg-zinc-100 px-1 py-0.5 text-[10px]">POST /api/auth/change-password</code>
        </p>
        <div className="grid gap-4">
          <div className="grid gap-1.5">
            <label className="text-xs font-medium text-zinc-500" htmlFor="modal-currentPw">
              รหัสผ่านปัจจุบัน
            </label>
            <Input
              id="modal-currentPw"
          ...
          <div className="grid gap-1.5">
            <label className="text-xs font-medium text-zinc-500" htmlFor="modal-newPw">
              รหัสผ่านใหม่
            </label>
            <Input
              id="modal-newPw"
          ...
          <div className="grid gap-1.5">
            <label className="text-xs font-medium text-zinc-500" htmlFor="modal-confirmPw">
              ยืนยันรหัสผ่านใหม่
            </label>
            <Input
              id="modal-confirmPw"
          ...
          <div className="flex flex-wrap gap-2 pt-1">
            ...
            <Button type="button" variant="ghost" className="text-zinc-600" onClick={closePwModal}>
              ยกเลิก
            </Button>
          </div>
        </div>
